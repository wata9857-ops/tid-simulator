...
const REF = {
    ...
    '甲子園口':   { cross: 1, pairs: [['Down_In', 'Up_In', 'l', 'L']],
                   src: '698 折返線 (2番) は立花方で下り内から入り上り内へ出る。西宮方は行き止まり。下り外はホームなし' },
    ...
    '吹田':   { cross: 1, pairs: [['Down_In', 'Up_In', 'x', 'R']], junctions: 0, stubs: 0,
                src: '696 東淀川側に 下り内↔上り内 の両渡り。本線と貨物線 (北方貨物線) はここではつながらない。吹田タは北方貨物線の帯' },
    '岸辺':   { cross: 0, stubs: 1, src: '696 吹田総合車両所は吹田側の下' },
    '千里丘': { none: true, src: '696 相対式2面2線' },
    '東淀川': { none: true, src: '697 相対式2面2線' },
    '茨木':   { cross: 2, pairs: [['Up_Out', 'Up_In', 'x', 'R'], ['Down_In', 'Down_Out', 'r', 'R']], junctions: 2,
                stubs: 1, src: '695 渡り線は千里丘側。千里丘側で列車線から貨物線 (北方貨物線の帯) が分かれ、大阪貨物ターミナル方が上へ' },
    ...
    '放出':       { cross: 1, stubs: 2, src: '712 おおさか東線・電留線は徳庵側 (片町線は線路図の中になった)' },

    // ================================ 山陽本線 姫路より西・赤穂線  画像(723)〜(725)
    '手柄山平和公園': { none: true, src: '724 相対式2面2線。渡り線なし' },
    '英賀保':   { cross: 2, pairs: [['Up_Out', 'Down_Out', 'x', 'L'], ['Up_Out', 'Down_Out', 'x', 'R']], stubs: 1,
                  src: '724 2面3線。中線が両端で上下本線につながる' },
    'はりま勝原': { none: true, src: '724 相対式2面2線' },
    '網干':     { cross: 2, stubs: 2, src: '725 2面3線。折返し線が両端で上下本線、竜野方の南に網干総合車両所' },
    '竜野':     { none: true, src: '725 島式＋下りの待避線。上下をつなぐ渡り線なし' },
    '相生':     { cross: 2, junctions: 2, stubs: 1, src: '725/723 2面3線。赤穂線は有年方で下り本線から分かれる' },
    '有年':     { none: true, src: '725 相対式2面2線' },
    '上郡':     { cross: 2, stubs: 3, src: '725 2面3線＋待避線。智頭急行線は有年と反対側' },
    '西相生':   { none: true, src: '723 単線の1線' },
    '坂越':     { none: true, src: '723 単線の1線' },
    '播州赤穂': { cross: 1, stubs: 1, src: '723 2面3線。3線とも相生方でまとまる' },

    // ================================ 学研都市線 放出〜木津  画像(726)〜(728)
    '徳庵':     { cross: 1, pairs: [['Tozai_Up', 'Tozai_Down', 'l', 'R']], stubs: 2,
                  src: '726 2面3線。放出方に片渡り、近畿車輛専用線と放出電留線' },
    '鴻池新田': { none: true, src: '727 相対式2面2線' },
    '住道':     { none: true, src: '727 2面4線。待避線だけ' },
    '野崎':     { none: true, src: '727 相対式2面2線' },
    '四条畷':   { cross: 2, stubs: 1, src: '727 2面4線。木津方に片渡り、放出方に両渡り' },
    '忍ケ丘':   { none: true, src: '727 相対式2面2線' },
    '寝屋川公園': { none: true, src: '727 島式1面2線' },
    '星田':     { none: true, src: '727 相対式2面2線' },
    '河内磐船': { none: true, src: '727 相対式2面2線' },
    '津田':     { none: true, stubs: 1, src: '727 相対式2面2線＋側線' },
    '藤阪':     { none: true, src: '727 島式1面2線' },
    '長尾':     { cross: 2, src: '727 島式1面2線。木津方に片渡り、放出方に両渡り' },
    '松井山手': { cross: 2, stubs: 1, src: '727 相対式2面2線。放出方に両渡り、木津方で単線へ' },
    '大住':     { cross: 2, src: '727 単線の交換駅' },
    '京田辺':   { cross: 2, src: '728 単線の交換駅 (4線)' },
    '同志社前': { none: true, src: '728 単線の1線' },
    'JR三山木': { cross: 2, src: '728 単線の交換駅' },
    '下狛':     { none: true, src: '728 単線の1線' },
    '祝園':     { cross: 2, stubs: 1, src: '728 単線の交換駅。放出方に両渡り、側線' },
    '西木津':   { none: true, src: '728 単線の1線' },
    '木津':     { none: true, stubs: 1, src: '728 学研都市線のホームは1線' }
};
...
head('折り返す駅に方転できる設備があるか');
{
    ...
    Object.keys(need).forEach(st => {
        ...
        // 単線上の1線の駅は、線路が1本なのでそのまま向きを変えられる
        const singleLine = !!STATION_REVERSE_SINGLE_LINE[st];
        if (!hasCross && !hasDrawUp && !singleLine && !KNOWN_NO_REVERSE[st]) bad.push(st);
    });
