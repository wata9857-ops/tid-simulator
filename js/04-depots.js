...
const DEPOTS = {
    ...
    /* ★網干総合車両所 (配線略図 スクリーンショット(725).png)。
       網干駅の竜野方、下り本線の南側に広がる大きな車両所で、
       網干駅の下り側 (2番・3番) から出入りする。
       姫路より西を線路図に入れたので、姫路電留線の別名ではなく実在の位置に置く。 */
    "網干": { capacity: 8, trains: [], drawOffset: { x: 0.5, y: 40 }, display: "網干〜竜野間 (網干総合車両所)" },
    ...
    /* ★JR東西線・片町線(学研都市線)の車両を受け持つ放出の電留線。
       電留線は放出駅の四条畷方 (放出〜徳庵間) にある。
       以前は線路図で放出の京橋方 (鴫野との間) に描いていて、実物と逆だった。
...
/* 留置場の別名。
   (以前は網干総合車両所や松井山手の電留線が線路図の範囲の外にあったので、
   線路図の上では、いちばん近い駅の留置線から出入りするものとして扱う。
   出入区の処理がどちらの名前で呼ばれても同じ留置場を指すようにしていた) */
/* ★姫路より西 (網干・播州赤穂・上郡) と学研都市線 (松井山手・四条畷) は
   線路図に入れたので、別名はもう要らない。網干は実在の網干総合車両所。 */
const DEPOT_ALIAS = {};
...
const DEPOT_LAYOUTS = {
    "姫路": {
        ...
        groups: [
            ...
            /* 網干総合車両所は姫路より西を線路図に入れたので、
               網干の構内図 (DEPOT_LAYOUTS["網干"]) に移した。 */        ]
    },

    "網干": {
        title: "網干総合車両所",
        owner: "網干総合車両所",
        leftLabel: "はりま勝原・姫路方 / 網干駅",
        rightLabel: "竜野・相生方",
        note: "配線略図 (スクリーンショット(725).png) のとおり、網干駅の竜野方、" +
              "下り本線の南側に電留線・検修庫・洗浄線が櫛状に並ぶ。" +
              "網干駅の2番・3番 (下り本線・折返し線) と出入区線でつながる。" +
              "新快速 (223系・225系 8両＋4両) の本拠。",
        groups: [
            { name: "網干駅 電留線 (駅南側)", tracks: [
                { label: "駅電1", cars: 12, kind: "stabling" },
                { label: "駅電2", cars: 12, kind: "stabling" },
                { label: "駅電3", cars: 8, kind: "stabling" },
                { label: "駅電4", cars: 8, kind: "stabling" }
            ]},
            /* ★網干総合車両所は新快速 (8両＋4両) の本拠。
               実際の網干は電留線20本以上の大規模な車両所なので、
               配線略図 (スクリーンショット(707)/(708).png の網干) に合わせて
               本数を増やした。ここが足りないと、在籍する編成の6割以上が
               「その他 / 構内留置」として図の外に出てしまう。 */
            { name: "網干総合車両所 電留線・検修庫", tracks: [
    ...
    "放出": {
        ...
        owner: "網干総合車両所明石支所",
        leftLabel: "放出駅 (京橋・JR東西線方) / おおさか東線",
        rightLabel: "徳庵・四条畷方 / 近畿車輛専用線",
        note: "放出駅の東方 (四条畷方)、徳庵との間に広がる電留線群。" +
              "JR東西線・片町線(学研都市線)の207系・321系が夜間滞泊する。" +
              "配線略図のとおり、本線南側に14本の留置線が櫛状に並び、" +
              "西端で放出駅の1〜4番のりばへつながる。" +
              "尼崎・JR東西線方面へ出区する列車は放出駅を京橋方へ抜ける。",
        groups: [
            { name: "出入区線", tracks: [
